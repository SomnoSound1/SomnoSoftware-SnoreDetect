...
{
    public class FourierTransform_alt
    {
        ...
        static public double[] FFTDb(double[] x)
        {
            ...
            {
                for (int i = 0; i < n / 2; i++)
                    decibel[i] = Statics.dB_factor * Math.Log10((float)(Math.Sqrt((xre[i] * xre[i]) + (xim[i] * xim[i]))));
                return decibel;
            }
            else
            {
                for (int i = 0; i < n / 2; i++)
                    linear[i] = (double)(Math.Sqrt((xre[i] * xre[i]) + (xim[i] * xim[i])));
                return linear;
            }
        }
